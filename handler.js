                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Bienvenido, @user').replace('@group', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'Desconocido') :
                            ...
                            let wel = API('lol', '/api/base/welcome', {
                                ...
                                profile: 'https://telegra.ph/file/a0d4dadf97dc29fba2c32.jpg',
                                background: 'https://i.imgur.com/bbWbASn.jpg'
                            }, 'apikey')

                            let lea = API('lol', '/api/base/leave', {
                                username: await this.getName(user),
                                groupname: await this.getName(id),
                                groupicon: ppgp,
                                membercount: groupMetadata.participants.length,
                        ...
                        this.sendFile(id, action === 'add' ? wel : lea, 'SF-MD BY @AdminSF', text, null, false, { mentions: [user] })
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || conn.sRevoke || 'Perubahan tautan grup a\n@revoke').replace('@revoke', groupUpdate.revoke)
        if (!text) continue
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        group: '⚙️ Perintah ini hanya dapat digunakan didalam grup!',
        ...
        unreg: '📇 Daftar untuk menggunakan Bot Dengan menulis:\n\n*/reg name.age*\n\n📌Contoh: */reg adminsf.20*',
